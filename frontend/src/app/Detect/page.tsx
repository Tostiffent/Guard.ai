    ...
    try {
      ...
      if (file.type.startsWith("image/")) {
        response = await axios.post(
          "https://fantastic-couscous-97qr9rgr4964fx9p5-5000.app.github.dev/detect_image",
          formData
        );
      } else {
        response = await axios.post(
  ...
  return (
    <div className={`min-h-screen ${theme === "dark" ? "bg-black text-white" : "bg-white text-black"}`}>
      <header
        ...
        } backdrop-blur-md transition-transform duration-300 ${showNavbar ? "translate-y-0" : "-translate-y-full"}`}
      >
        <nav className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex space-x-6 items-center">
            ...
            <Link href="/Protect" className="text-sm font-medium hover:text-blue-400 transition-colors">
              Protect
            </Link>
        ...
        >
          ...
          <div className="flex flex-wrap justify-between items-center mt-4">
            <FileInputButton
              ...
              } text-white transition-colors mb-2 sm:mb-0`}
            >
              Select File
            </FileInputButton>
            {files.length === 1 && (
      ...
      <footer className={`mt-12 py-8 ${theme === "dark" ? "bg-gray-900" : "bg-gray-100"} translate-y-[60px]`}>
        <div className="container mx-auto px-4 text-center">
          <p className={`${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
            &copy; 2024 guard.ai. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
